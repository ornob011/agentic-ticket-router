-- =====================================================
-- Agentic Router Support Ticket System - PostgreSQL DDL
-- (NO Postgres ENUM types; uses VARCHAR + CHECK constraints)
-- =====================================================

-- Enable the base vector engine
CREATE EXTENSION IF NOT EXISTS vector;

-- Drop existing objects (in reverse dependency order)
DROP TABLE IF EXISTS notification CASCADE;
DROP TABLE IF EXISTS audit_event CASCADE;
DROP TABLE IF EXISTS routing_pattern_keyword CASCADE;
DROP TABLE IF EXISTS routing_pattern CASCADE;
DROP TABLE IF EXISTS resolution_feedback CASCADE;
DROP TABLE IF EXISTS ticket_routing CASCADE;
DROP TABLE IF EXISTS ticket_message CASCADE;
DROP TABLE IF EXISTS escalation CASCADE;
DROP TABLE IF EXISTS agent_runtime_step CASCADE;
DROP TABLE IF EXISTS agent_runtime_run CASCADE;
DROP TABLE IF EXISTS llm_output CASCADE;
DROP TABLE IF EXISTS support_ticket CASCADE;
DROP TABLE IF EXISTS agent_queue_membership CASCADE;
DROP TABLE IF EXISTS customer_profile CASCADE;
DROP TABLE IF EXISTS user_settings CASCADE;
DROP TABLE IF EXISTS app_user CASCADE;
DROP TABLE IF EXISTS model_registry CASCADE;
DROP TABLE IF EXISTS policy_config CASCADE;
DROP TABLE IF EXISTS customer_tier CASCADE;
DROP TABLE IF EXISTS country CASCADE;
DROP TABLE IF EXISTS language CASCADE;
DROP TABLE IF EXISTS article_template_applicable_priorities CASCADE;
DROP TABLE IF EXISTS article_template_applicable_categories CASCADE;
DROP TABLE IF EXISTS article_template CASCADE;
DROP TABLE IF EXISTS knowledge_article CASCADE;
DROP TABLE IF EXISTS vector_store;

-- Drop sequences
DROP SEQUENCE IF EXISTS support_ticket_ticket_no_seq CASCADE;

-- =====================================================
-- REFERENCE TABLES
-- =====================================================

-- Language table
CREATE TABLE language
(
  code VARCHAR(10)  NOT NULL PRIMARY KEY,
  name VARCHAR(100) NOT NULL,

  CONSTRAINT chk_language_code_format CHECK (code ~ '^[a-z]{2}(-[A-Z]{2})?$')
);

CREATE INDEX idx_language_name ON language (name);

COMMENT ON TABLE language IS 'ISO language codes and names';
COMMENT ON COLUMN language.code IS 'Language code in format: en, bn, or en-GB';

-- Country table
CREATE TABLE country
(
  iso2   CHAR(2)      NOT NULL PRIMARY KEY,
  name   VARCHAR(100) NOT NULL,
  active BOOLEAN      NOT NULL DEFAULT true,

  CONSTRAINT chk_country_iso2_format CHECK (iso2 ~ '^[A-Za-z]{2}$')
);

CREATE INDEX idx_country_name ON country (name);
CREATE INDEX idx_country_active ON country (active);

COMMENT ON TABLE country IS 'ISO 3166-1 alpha-2 country codes';

-- Customer Tier table
CREATE TABLE customer_tier
(
  code         VARCHAR(50)  NOT NULL PRIMARY KEY,
  display_name VARCHAR(100) NOT NULL,
  active       BOOLEAN      NOT NULL DEFAULT true,

  CONSTRAINT chk_customer_tier_code_format CHECK (code ~ '^[A-Z0-9_]+$')
);

CREATE INDEX idx_customer_tier_active ON customer_tier (active);

COMMENT ON TABLE customer_tier IS 'Customer service tier levels (FREE, STANDARD, PREMIUM, etc.)';

-- =====================================================
-- CORE APPLICATION TABLES
-- =====================================================

-- App User table
CREATE TABLE app_user
(
  id             BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version    BIGINT       NOT NULL DEFAULT 0,
  created_at     TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at     TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by     VARCHAR(100),
  updated_by     VARCHAR(100),

  username       VARCHAR(50)  NOT NULL UNIQUE,
  email          VARCHAR(100) NOT NULL UNIQUE,
  password_hash  VARCHAR(512) NOT NULL,
  full_name      VARCHAR(100),
  role           VARCHAR(20)  NOT NULL,

  active         BOOLEAN      NOT NULL DEFAULT true,
  email_verified BOOLEAN      NOT NULL DEFAULT false,
  last_login_at  TIMESTAMPTZ,
  last_login_ip  VARCHAR(45),

  CONSTRAINT chk_app_user_role
    CHECK (role IN ('CUSTOMER', 'AGENT', 'SUPERVISOR', 'ADMIN'))
);

CREATE UNIQUE INDEX idx_app_user_email ON app_user (email);
CREATE UNIQUE INDEX idx_app_user_username ON app_user (username);
CREATE INDEX idx_app_user_role ON app_user (role);
CREATE INDEX idx_app_user_active_role ON app_user (active, role);

COMMENT ON TABLE app_user IS 'Application users including customers, agents, supervisors, and admins';
COMMENT ON COLUMN app_user.password_hash IS 'Bcrypt or similar hashed password';

-- Customer Profile table
CREATE TABLE customer_profile
(
  id                      BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version             BIGINT      NOT NULL DEFAULT 0,
  created_at              TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at              TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by              VARCHAR(100),
  updated_by              VARCHAR(100),

  user_id                 BIGINT      NOT NULL UNIQUE,
  company_name            VARCHAR(100),
  phone_number            VARCHAR(20),
  address                 VARCHAR(255),
  city                    VARCHAR(100),
  country_iso2            CHAR(2),
  postal_code             VARCHAR(20),
  tier_code               VARCHAR(50),
  preferred_language_code VARCHAR(10),
  notifications_enabled   BOOLEAN     NOT NULL DEFAULT true,

  CONSTRAINT fk_customer_profile_user FOREIGN KEY (user_id)
    REFERENCES app_user (id) ON DELETE CASCADE,
  CONSTRAINT fk_customer_profile_country FOREIGN KEY (country_iso2)
    REFERENCES country (iso2) ON DELETE SET NULL,
  CONSTRAINT fk_customer_profile_tier FOREIGN KEY (tier_code)
    REFERENCES customer_tier (code) ON DELETE SET NULL,
  CONSTRAINT fk_customer_profile_language FOREIGN KEY (preferred_language_code)
    REFERENCES language (code) ON DELETE SET NULL
);

CREATE UNIQUE INDEX idx_customer_profile_user_id ON customer_profile (user_id);
CREATE INDEX idx_customer_profile_country ON customer_profile (country_iso2);
CREATE INDEX idx_customer_profile_tier ON customer_profile (tier_code);

COMMENT ON TABLE customer_profile IS 'Extended profile information for customer users';

-- Policy Config table
CREATE TABLE policy_config
(
  id            BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version   BIGINT       NOT NULL DEFAULT 0,
  created_at    TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at    TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by    VARCHAR(100),
  updated_by    VARCHAR(100),
  config_key    VARCHAR(100) NOT NULL UNIQUE,
  config_value  NUMERIC(20, 6),
  description   TEXT,
  active        BOOLEAN      NOT NULL DEFAULT true,
  default_value NUMERIC(20, 6),
  min_value     NUMERIC(20, 6),
  max_value     NUMERIC(20, 6),

  CONSTRAINT chk_policy_config_key
    CHECK (config_key IN (
                          'AUTO_ROUTE_THRESHOLD',
                          'CRITICAL_MIN_CONF',
                          'ROUTER_REPAIR_MAX_RETRIES',
                          'SLA_ASSIGNED_HOURS_HIGH',
                          'WAITING_CUSTOMER_REMINDER_HOURS',
                          'INACTIVITY_AUTO_CLOSE_DAYS',
                          'SLA_CUSTOMER_RESPONSE_HOURS',
                          'SLA_AGENT_RESPONSE_HOURS',
                          'AUTO_CLOSE_WARNING_DAYS',
                          'AUTO_CLOSE_FINAL_DAYS',
                          'MAX_ATTACHMENT_BYTES',
                          'AUTO_CLOSE_ENABLED',
                          'DEFAULT_QUEUE',
                          'ROUTER_MODEL_PARAMS',
                          'MAX_AUTONOMOUS_ACTIONS',
                          'MAX_QUESTIONS_PER_TICKET',
                          'AUTONOMOUS_ENABLED',
                          'FRUSTRATION_DETECTION_ENABLED',
                          'LOOP_DETECTION_ENABLED'
      ))
);

CREATE INDEX idx_policy_config_active ON policy_config (active);

COMMENT ON TABLE policy_config IS 'System-wide policy configuration settings';
COMMENT ON COLUMN policy_config.config_key IS 'Unique configuration key (e.g., AUTO_CLOSE_DAYS, CONFIDENCE_THRESHOLD)';

-- Model Registry table
CREATE TABLE model_registry
(
  id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version         BIGINT       NOT NULL DEFAULT 0,
  created_at          TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at          TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by          VARCHAR(100),
  updated_by          VARCHAR(100),

  model_tag           VARCHAR(100) NOT NULL UNIQUE,
  model_name          VARCHAR(200) NOT NULL,
  version             VARCHAR(50)  NOT NULL,
  description         TEXT,
  base_model          VARCHAR(50),
  training_method     VARCHAR(50),
  quantization        VARCHAR(50),
  artifact_path       VARCHAR(500),
  active              BOOLEAN      NOT NULL DEFAULT false,
  is_default          BOOLEAN      NOT NULL DEFAULT false,
  activated_at        TIMESTAMPTZ,
  deactivated_at      TIMESTAMPTZ,
  activated_by_id     BIGINT,
  performance_metrics JSONB,
  model_config        JSONB,

  CONSTRAINT fk_model_registry_activated_by FOREIGN KEY (activated_by_id)
    REFERENCES app_user (id) ON DELETE SET NULL
);

CREATE INDEX idx_model_registry_active ON model_registry (active);
CREATE INDEX idx_model_registry_active_default ON model_registry (active, is_default);

COMMENT ON TABLE model_registry IS 'Registry of AI models used for ticket routing and classification';

-- Support Ticket table
CREATE SEQUENCE support_ticket_ticket_no_seq START WITH 1 INCREMENT BY 1;

CREATE TABLE support_ticket
(
  id                        BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version               BIGINT       NOT NULL        DEFAULT 0,
  created_at                TIMESTAMPTZ  NOT NULL        DEFAULT CURRENT_TIMESTAMP,
  updated_at                TIMESTAMPTZ  NOT NULL        DEFAULT CURRENT_TIMESTAMP,
  created_by                VARCHAR(100),
  updated_by                VARCHAR(100),

  ticket_no                 BIGINT       NOT NULL UNIQUE DEFAULT nextval('support_ticket_ticket_no_seq'),
  customer_id               BIGINT       NOT NULL,
  subject                   VARCHAR(255) NOT NULL,
  status                    VARCHAR(40)  NOT NULL        DEFAULT 'RECEIVED',
  current_category          VARCHAR(30),
  current_priority          VARCHAR(20)  NOT NULL        DEFAULT 'MEDIUM',
  assigned_queue            VARCHAR(30),
  assigned_agent_id         BIGINT,
  last_activity_at          TIMESTAMPTZ  NOT NULL        DEFAULT CURRENT_TIMESTAMP,
  first_assigned_at         TIMESTAMPTZ,
  resolved_at               TIMESTAMPTZ,
  closed_at                 TIMESTAMPTZ,
  reopen_count              INT          NOT NULL        DEFAULT 0,
  escalated                 BOOLEAN      NOT NULL        DEFAULT false,
  requires_human_review     BOOLEAN      NOT NULL        DEFAULT false,
  latest_routing_confidence NUMERIC(5, 4),
  latest_routing_version    INT          NOT NULL        DEFAULT 0,
  metadata                  JSONB,

  CONSTRAINT fk_support_ticket_customer FOREIGN KEY (customer_id)
    REFERENCES app_user (id) ON DELETE RESTRICT,
  CONSTRAINT fk_support_ticket_assigned_agent FOREIGN KEY (assigned_agent_id)
    REFERENCES app_user (id) ON DELETE SET NULL,

  CONSTRAINT chk_support_ticket_status
    CHECK (status IN (
                      'RECEIVED',
                      'TRIAGING',
                      'WAITING_CUSTOMER',
                      'ASSIGNED',
                      'IN_PROGRESS',
                      'RESOLVED',
                      'ESCALATED',
                      'AUTO_ESCALATED',
                      'AUTO_CLOSED_PENDING',
                      'CLOSED'
      )),

  CONSTRAINT chk_support_ticket_category
    CHECK (current_category IS NULL OR current_category IN (
                                                            'BILLING',
                                                            'TECHNICAL',
                                                            'ACCOUNT',
                                                            'SHIPPING',
                                                            'SECURITY',
                                                            'PRICING',
                                                            'OTHER',
                                                            'CANCEL',
                                                            'CONTACT',
                                                            'DELIVERY',
                                                            'FEEDBACK',
                                                            'INVOICE',
                                                            'ORDER',
                                                            'PAYMENT',
                                                            'REFUND',
                                                            'SUBSCRIPTION'
      )),

  CONSTRAINT chk_support_ticket_priority
    CHECK (current_priority IN ('CRITICAL', 'HIGH', 'MEDIUM', 'LOW')),

  CONSTRAINT chk_support_ticket_queue
    CHECK (assigned_queue IS NULL OR assigned_queue IN (
                                                        'BILLING_Q',
                                                        'TECH_Q',
                                                        'OPS_Q',
                                                        'SECURITY_Q',
                                                        'ACCOUNT_Q',
                                                        'GENERAL_Q'
      )),

  CONSTRAINT chk_support_ticket_confidence_range
    CHECK (latest_routing_confidence IS NULL OR
           (latest_routing_confidence >= 0 AND latest_routing_confidence <= 1))
);

CREATE UNIQUE INDEX idx_support_ticket_ticket_no ON support_ticket (ticket_no);
CREATE INDEX idx_support_ticket_customer_id ON support_ticket (customer_id);
CREATE INDEX idx_support_ticket_status ON support_ticket (status);
CREATE INDEX idx_support_ticket_assigned_queue ON support_ticket (assigned_queue);
CREATE INDEX idx_support_ticket_current_priority ON support_ticket (current_priority);
CREATE INDEX idx_support_ticket_last_activity_at ON support_ticket (last_activity_at);
CREATE INDEX idx_support_ticket_assigned_agent_id ON support_ticket (assigned_agent_id);
CREATE INDEX idx_support_ticket_status_queue_priority ON support_ticket (status, assigned_queue, current_priority);
CREATE INDEX idx_support_ticket_customer_created ON support_ticket (customer_id, created_at);

COMMENT ON TABLE support_ticket IS 'Main support ticket entity';
COMMENT ON COLUMN support_ticket.ticket_no IS 'Human-readable sequential ticket number';

-- Agent Queue Membership table
CREATE TABLE agent_queue_membership
(
  id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version BIGINT      NOT NULL DEFAULT 0,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by  VARCHAR(100),
  updated_by  VARCHAR(100),
  user_id     BIGINT      NOT NULL,
  queue       VARCHAR(30) NOT NULL,

  CONSTRAINT fk_agent_queue_membership_user FOREIGN KEY (user_id)
    REFERENCES app_user (id) ON DELETE CASCADE,
  CONSTRAINT uk_agent_queue_membership_user_queue UNIQUE (user_id, queue),
  CONSTRAINT chk_agent_queue_membership_queue
    CHECK (queue IN (
                     'BILLING_Q',
                     'TECH_Q',
                     'OPS_Q',
                     'SECURITY_Q',
                     'ACCOUNT_Q',
                     'GENERAL_Q'
      ))
);

CREATE INDEX idx_agent_queue_membership_user_id ON agent_queue_membership (user_id);
CREATE INDEX idx_agent_queue_membership_queue ON agent_queue_membership (queue);

-- LLM Output table
CREATE TABLE llm_output
(
  id               BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version      BIGINT       NOT NULL DEFAULT 0,
  created_at       TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at       TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by       VARCHAR(100),
  updated_by       VARCHAR(100),

  ticket_id        BIGINT       NOT NULL,
  model_tag        VARCHAR(100) NOT NULL,
  output_type      VARCHAR(60)  NOT NULL DEFAULT 'ROUTING',
  raw_request      JSONB        NOT NULL,
  raw_response     JSONB,
  parse_status     VARCHAR(40)  NOT NULL,

  error_message    TEXT,
  latency_ms       BIGINT,
  repair_attempts  INT          NOT NULL DEFAULT 0,
  temperature      DOUBLE PRECISION,
  max_tokens       INT,
  inference_config JSONB,

  CONSTRAINT fk_llm_output_ticket FOREIGN KEY (ticket_id)
    REFERENCES support_ticket (id) ON DELETE CASCADE,

  CONSTRAINT chk_llm_output_type
    CHECK (output_type IN (
                           'ROUTING',
                           'ANALYSIS',
                           'CATEGORY_DETECTION'
      )),

  CONSTRAINT chk_llm_parse_status
    CHECK (parse_status IN (
                            'SUCCESS',
                            'INVALID_JSON',
                            'INVALID_ENUM',
                            'SCHEMA_VIOLATION',
                            'REPAIR_ATTEMPTED',
                            'REPAIR_SUCCESS',
                            'REPAIR_FAILED',
                            'TIMEOUT',
                            'MODEL_ERROR'
      ))
);

CREATE INDEX idx_llm_output_ticket_id ON llm_output (ticket_id);
CREATE INDEX idx_llm_output_created_at ON llm_output (created_at);
CREATE INDEX idx_llm_output_parse_status ON llm_output (parse_status);
CREATE INDEX idx_llm_output_model_tag ON llm_output (model_tag);
CREATE INDEX idx_llm_output_type ON llm_output (output_type);

COMMENT ON TABLE llm_output IS 'LLM inference requests and responses for ticket analysis';

-- Agent Runtime Run table
CREATE TABLE agent_runtime_run
(
  id                 BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version        BIGINT      NOT NULL DEFAULT 0,
  created_at         TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at         TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by         VARCHAR(100),
  updated_by         VARCHAR(100),

  ticket_id          BIGINT      NOT NULL,
  correlation_id     VARCHAR(100),
  status             VARCHAR(30) NOT NULL,
  termination_reason VARCHAR(50),
  total_steps        INTEGER,
  fallback_used      BOOLEAN     NOT NULL DEFAULT false,
  error_code         VARCHAR(100),
  error_message      TEXT,
  started_at         TIMESTAMPTZ NOT NULL,
  ended_at           TIMESTAMPTZ,

  CONSTRAINT fk_agent_runtime_run_ticket FOREIGN KEY (ticket_id)
    REFERENCES support_ticket (id) ON DELETE CASCADE,
  CONSTRAINT chk_agent_runtime_run_status
    CHECK (status IN (
                      'RUNNING',
                      'COMPLETED',
                      'FAILED'
      )),
  CONSTRAINT chk_agent_runtime_termination_reason
    CHECK (termination_reason IS NULL OR termination_reason IN (
                                                                'GOAL_REACHED',
                                                                'SAFETY_BLOCKED',
                                                                'BUDGET_EXCEEDED',
                                                                'RUNTIME_TIMEOUT',
                                                                'PLAN_VALIDATION_FAILED',
                                                                'TOOL_FAILURE'
      ))
);

CREATE INDEX idx_agent_runtime_run_ticket_id ON agent_runtime_run (ticket_id);
CREATE INDEX idx_agent_runtime_run_status ON agent_runtime_run (status);
CREATE INDEX idx_agent_runtime_run_correlation_id ON agent_runtime_run (correlation_id);

COMMENT ON TABLE agent_runtime_run IS 'Agent runtime execution runs per ticket';

-- Agent Runtime Step table
CREATE TABLE agent_runtime_step
(
  id                 BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version        BIGINT      NOT NULL DEFAULT 0,
  created_at         TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at         TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by         VARCHAR(100),
  updated_by         VARCHAR(100),

  run_id             BIGINT      NOT NULL,
  step_no            INTEGER     NOT NULL,
  step_type          VARCHAR(30) NOT NULL,
  planner_output     JSONB,
  validated_response JSONB,
  safety_decision    JSONB,
  tool_result        JSONB,
  latency_ms         BIGINT,
  success            BOOLEAN     NOT NULL DEFAULT true,
  error_code         VARCHAR(100),
  error_message      TEXT,

  CONSTRAINT fk_agent_runtime_step_run FOREIGN KEY (run_id)
    REFERENCES agent_runtime_run (id) ON DELETE CASCADE,
  CONSTRAINT chk_agent_runtime_step_type
    CHECK (step_type IN (
                         'PLAN',
                         'SAFETY',
                         'ACT',
                         'REFLECT',
                         'TERMINATE'
      ))
);

CREATE INDEX idx_agent_runtime_step_run_id ON agent_runtime_step (run_id);
CREATE INDEX idx_agent_runtime_step_step_no ON agent_runtime_step (step_no);
CREATE INDEX idx_agent_runtime_step_step_type ON agent_runtime_step (step_type);

COMMENT ON TABLE agent_runtime_step IS 'Step-level trace of an agent runtime run';

-- Ticket Message table
CREATE TABLE ticket_message
(
  id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version         BIGINT      NOT NULL DEFAULT 0,
  created_at          TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by          VARCHAR(100),
  updated_by          VARCHAR(100),

  ticket_id           BIGINT      NOT NULL,
  author_id           BIGINT,
  message_kind        VARCHAR(40) NOT NULL,
  content             TEXT        NOT NULL,
  visible_to_customer BOOLEAN     NOT NULL DEFAULT true,
  llm_output_id       BIGINT,
  read_by_customer    BOOLEAN     NOT NULL DEFAULT false,
  read_by_agent       BOOLEAN     NOT NULL DEFAULT false,

  CONSTRAINT fk_ticket_message_ticket FOREIGN KEY (ticket_id)
    REFERENCES support_ticket (id) ON DELETE CASCADE,
  CONSTRAINT fk_ticket_message_author FOREIGN KEY (author_id)
    REFERENCES app_user (id) ON DELETE SET NULL,
  CONSTRAINT fk_ticket_message_llm_output FOREIGN KEY (llm_output_id)
    REFERENCES llm_output (id) ON DELETE SET NULL,

  CONSTRAINT chk_ticket_message_kind
    CHECK (message_kind IN (
                            'CUSTOMER_MESSAGE',
                            'AGENT_MESSAGE',
                            'SYSTEM_MESSAGE',
                            'CLARIFYING_QUESTION',
                            'AUTO_REPLY',
                            'INTERNAL_NOTE'
      ))
);

CREATE INDEX idx_ticket_message_ticket_id ON ticket_message (ticket_id);
CREATE INDEX idx_ticket_message_created_at ON ticket_message (created_at);
CREATE INDEX idx_ticket_message_ticket_created ON ticket_message (ticket_id, created_at);

COMMENT ON TABLE ticket_message IS 'Messages and communications on support tickets';

-- Ticket Routing table
CREATE TABLE ticket_routing
(
  id                    BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version           BIGINT        NOT NULL DEFAULT 0,
  created_at            TIMESTAMPTZ   NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at            TIMESTAMPTZ   NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by            VARCHAR(100),
  updated_by            VARCHAR(100),

  ticket_id             BIGINT        NOT NULL,
  version               INT           NOT NULL,
  llm_output_id         BIGINT,
  category              VARCHAR(30)   NOT NULL,
  priority              VARCHAR(20)   NOT NULL,
  queue                 VARCHAR(30)   NOT NULL,
  next_action           VARCHAR(30)   NOT NULL,
  confidence            NUMERIC(5, 4) NOT NULL,
  clarifying_question   TEXT,
  draft_reply           TEXT,
  rationale_tags        JSONB                  DEFAULT '[]'::jsonb,
  overridden            BOOLEAN       NOT NULL DEFAULT false,
  overridden_by_id      BIGINT,
  override_reason       TEXT,
  policy_gate_triggered BOOLEAN       NOT NULL DEFAULT false,
  triggered_policy_name VARCHAR(100),
  applied               BOOLEAN       NOT NULL DEFAULT false,
  metadata              JSONB,

  CONSTRAINT fk_ticket_routing_ticket FOREIGN KEY (ticket_id)
    REFERENCES support_ticket (id) ON DELETE CASCADE,
  CONSTRAINT fk_ticket_routing_llm_output FOREIGN KEY (llm_output_id)
    REFERENCES llm_output (id) ON DELETE SET NULL,
  CONSTRAINT fk_ticket_routing_overridden_by FOREIGN KEY (overridden_by_id)
    REFERENCES app_user (id) ON DELETE SET NULL,

  CONSTRAINT chk_ticket_routing_confidence_range
    CHECK (confidence >= 0 AND confidence <= 1),

  CONSTRAINT chk_ticket_routing_category
    CHECK (category IN ('BILLING', 'TECHNICAL', 'ACCOUNT', 'SHIPPING', 'SECURITY', 'PRICING', 'OTHER')),

  CONSTRAINT chk_ticket_routing_priority
    CHECK (priority IN ('CRITICAL', 'HIGH', 'MEDIUM', 'LOW')),

  CONSTRAINT chk_ticket_routing_queue
    CHECK (queue IN ('BILLING_Q', 'TECH_Q', 'OPS_Q', 'SECURITY_Q', 'ACCOUNT_Q', 'GENERAL_Q')),

  CONSTRAINT chk_ticket_routing_next_action
    CHECK (next_action IN ('AUTO_REPLY', 'ASK_CLARIFYING', 'ASSIGN_QUEUE', 'ESCALATE', 'HUMAN_REVIEW',
                           'UPDATE_CUSTOMER_PROFILE', 'CHANGE_PRIORITY', 'ADD_INTERNAL_NOTE',
                           'AUTO_ESCALATE', 'AUTO_RESOLVE', 'REOPEN_TICKET', 'TRIGGER_NOTIFICATION',
                           'USE_KNOWLEDGE_ARTICLE', 'USE_TEMPLATE'))
);

CREATE INDEX idx_ticket_routing_ticket_id ON ticket_routing (ticket_id);
CREATE UNIQUE INDEX idx_ticket_routing_ticket_version ON ticket_routing (ticket_id, version);
CREATE INDEX idx_ticket_routing_llm_output_id ON ticket_routing (llm_output_id);

COMMENT ON TABLE ticket_routing IS 'AI routing decisions and overrides for tickets';
COMMENT ON COLUMN ticket_routing.version IS 'Routing version number for this ticket (incremental)';

-- Escalation table
CREATE TABLE escalation
(
  id                     BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version            BIGINT      NOT NULL DEFAULT 0,
  created_at             TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at             TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by             VARCHAR(100),
  updated_by             VARCHAR(100),

  ticket_id              BIGINT      NOT NULL UNIQUE,
  reason                 TEXT        NOT NULL,
  assigned_supervisor_id BIGINT,
  resolved               BOOLEAN     NOT NULL DEFAULT false,
  resolved_at            TIMESTAMPTZ,
  resolution_notes       TEXT,
  resolved_by_id         BIGINT,

  CONSTRAINT fk_escalation_ticket FOREIGN KEY (ticket_id)
    REFERENCES support_ticket (id) ON DELETE CASCADE,
  CONSTRAINT fk_escalation_supervisor FOREIGN KEY (assigned_supervisor_id)
    REFERENCES app_user (id) ON DELETE SET NULL,
  CONSTRAINT fk_escalation_resolved_by FOREIGN KEY (resolved_by_id)
    REFERENCES app_user (id) ON DELETE SET NULL
);

CREATE UNIQUE INDEX idx_escalation_ticket_id ON escalation (ticket_id);
CREATE INDEX idx_escalation_assigned_supervisor_id ON escalation (assigned_supervisor_id);
CREATE INDEX idx_escalation_resolved ON escalation (resolved);

COMMENT ON TABLE escalation IS 'Ticket escalations to supervisors or specialized teams';

-- Audit Event table
CREATE TABLE audit_event
(
  id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version     BIGINT      NOT NULL DEFAULT 0,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by      VARCHAR(100),
  updated_by      VARCHAR(100),
  ticket_id       BIGINT,
  event_type      VARCHAR(50) NOT NULL,
  performed_by_id BIGINT,
  description     TEXT        NOT NULL,
  payload         JSONB,
  ip_address      VARCHAR(45),
  user_agent      VARCHAR(500),
  correlation_id  VARCHAR(100),

  CONSTRAINT fk_audit_event_ticket FOREIGN KEY (ticket_id)
    REFERENCES support_ticket (id) ON DELETE CASCADE,
  CONSTRAINT fk_audit_event_performed_by FOREIGN KEY (performed_by_id)
    REFERENCES app_user (id) ON DELETE SET NULL,

  CONSTRAINT chk_audit_event_type
    CHECK (event_type IN (
                          'TICKET_CREATED',
                          'TICKET_STATUS_CHANGED',
                          'MESSAGE_POSTED',
                          'ROUTING_EXECUTED',
                          'ROUTING_OVERRIDDEN',
                          'QUEUE_ASSIGNED',
                          'AGENT_ASSIGNED',
                          'ESCALATION_CREATED',
                          'ESCALATION_RESOLVED',
                          'PRIORITY_CHANGED',
                          'NOTIFICATION_SENT',
                          'SLA_BREACH',
                          'AUTO_CLOSE_TRIGGERED',
                          'TICKET_REOPENED',
                          'POLICY_GATE_TRIGGERED',
                          'MODEL_INFERENCE_FAILED',
                          'MANUAL_INTERVENTION',
                          'TICKET_ANALYSIS_EXECUTED',
                          'TICKET_ANALYSIS_FAILED',
                          'TEMPLATE_AUTO_SELECTED'
      ))
);

CREATE INDEX idx_audit_event_ticket_id ON audit_event (ticket_id);
CREATE INDEX idx_audit_event_event_type ON audit_event (event_type);
CREATE INDEX idx_audit_event_performed_by_id ON audit_event (performed_by_id);
CREATE INDEX idx_audit_event_created_at ON audit_event (created_at);
CREATE INDEX idx_audit_event_ticket_created ON audit_event (ticket_id, created_at);

COMMENT ON TABLE audit_event IS 'Audit trail of all significant system events and user actions';

-- Notification table
CREATE TABLE notification
(
  id                BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version       BIGINT       NOT NULL DEFAULT 0,
  created_at        TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at        TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by        VARCHAR(100),
  updated_by        VARCHAR(100),
  recipient_id      BIGINT       NOT NULL,
  notification_type VARCHAR(40)  NOT NULL,
  title             VARCHAR(255) NOT NULL,
  body              TEXT         NOT NULL,
  ticket_id         BIGINT,
  link              VARCHAR(500),
  read              BOOLEAN      NOT NULL DEFAULT false,
  read_at           TIMESTAMPTZ,

  CONSTRAINT fk_notification_recipient FOREIGN KEY (recipient_id)
    REFERENCES app_user (id) ON DELETE CASCADE,
  CONSTRAINT fk_notification_ticket FOREIGN KEY (ticket_id)
    REFERENCES support_ticket (id) ON DELETE CASCADE,

  CONSTRAINT chk_notification_type
    CHECK (notification_type IN (
                                 'TICKET_ACK',
                                 'STATUS_CHANGE',
                                 'NEW_MESSAGE',
                                 'SLA_REMINDER',
                                 'ESCALATION',
                                 'AUTO_CLOSE_WARNING',
                                 'TICKET_REOPENED',
                                 'ASSIGNED_TO_YOU'
      ))
);

CREATE INDEX idx_notification_recipient_id ON notification (recipient_id);
CREATE INDEX idx_notification_read ON notification (read);
CREATE INDEX idx_notification_created_at ON notification (created_at);
CREATE INDEX idx_notification_recipient_read ON notification (recipient_id, read, created_at);
CREATE INDEX idx_notification_ticket_id ON notification (ticket_id);

COMMENT ON TABLE notification IS 'User notifications for ticket events and system updates';

-- Resolution Feedback table
CREATE TABLE resolution_feedback
(
  id                 BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version        BIGINT      NOT NULL DEFAULT 0,
  created_at         TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at         TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by         VARCHAR(100),
  updated_by         VARCHAR(100),
  ticket_id          BIGINT      NOT NULL,
  feedback_type      VARCHAR(20) NOT NULL,
  original_category  VARCHAR(30),
  corrected_category VARCHAR(30),
  original_action    VARCHAR(50),
  corrected_action   VARCHAR(50),
  rating             INTEGER,
  feedback_notes     TEXT,
  agent_id           BIGINT,
  routing_id         BIGINT,

  CONSTRAINT fk_resolution_feedback_ticket FOREIGN KEY (ticket_id)
    REFERENCES support_ticket (id) ON DELETE CASCADE,
  CONSTRAINT fk_resolution_feedback_agent FOREIGN KEY (agent_id)
    REFERENCES app_user (id) ON DELETE SET NULL,
  CONSTRAINT chk_resolution_feedback_type
    CHECK (feedback_type IN ('RATING', 'CORRECTION', 'REJECTION', 'APPROVAL')),
  CONSTRAINT chk_resolution_feedback_rating
    CHECK (rating IS NULL OR (rating >= 1 AND rating <= 5))
);

CREATE INDEX idx_feedback_ticket ON resolution_feedback (ticket_id);
CREATE INDEX idx_feedback_type ON resolution_feedback (feedback_type);
CREATE INDEX idx_feedback_agent ON resolution_feedback (agent_id);

COMMENT ON TABLE resolution_feedback IS 'Stores agent feedback on AI routing decisions';
COMMENT ON COLUMN resolution_feedback.feedback_type IS 'RATING, CORRECTION, REJECTION, or APPROVAL';
COMMENT ON COLUMN resolution_feedback.rating IS 'Rating from 1-5 for RATING feedback type';
COMMENT ON COLUMN resolution_feedback.corrected_action IS 'The action the agent chose instead';

-- Routing Pattern table
CREATE TABLE routing_pattern
(
  id                BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version       BIGINT      NOT NULL DEFAULT 0,
  created_at        TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at        TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by        VARCHAR(100),
  updated_by        VARCHAR(100),
  category          VARCHAR(30) NOT NULL,
  successful_action VARCHAR(50) NOT NULL,
  success_count     INTEGER     NOT NULL DEFAULT 1,
  failure_count     INTEGER     NOT NULL DEFAULT 0,
  last_success_at   TIMESTAMPTZ,
  last_failure_at   TIMESTAMPTZ,
  confidence_boost  DOUBLE PRECISION NOT NULL DEFAULT 0.1,
  active            BOOLEAN     NOT NULL DEFAULT TRUE
);

CREATE INDEX idx_pattern_category ON routing_pattern (category);
CREATE INDEX idx_pattern_action ON routing_pattern (successful_action);
CREATE INDEX idx_pattern_success ON routing_pattern (success_count);

COMMENT ON TABLE routing_pattern IS 'Stores learned routing patterns from successful resolutions';
COMMENT ON COLUMN routing_pattern.successful_action IS 'The action that worked for this pattern';
COMMENT ON COLUMN routing_pattern.confidence_boost IS 'How much to boost confidence when pattern matches';

-- Routing Pattern Keyword table
CREATE TABLE routing_pattern_keyword
(
  routing_pattern_id BIGINT       NOT NULL,
  keyword            VARCHAR(200) NOT NULL,

  CONSTRAINT fk_routing_pattern_keyword_pattern FOREIGN KEY (routing_pattern_id)
    REFERENCES routing_pattern (id) ON DELETE CASCADE,
  CONSTRAINT pk_routing_pattern_keyword PRIMARY KEY (routing_pattern_id, keyword)
);

CREATE INDEX idx_routing_pattern_keyword_keyword ON routing_pattern_keyword (keyword);

COMMENT ON TABLE routing_pattern_keyword IS 'Keywords associated with a routing pattern';
COMMENT ON COLUMN routing_pattern_keyword.keyword IS 'Keyword that indicates this routing pattern';

-- Knowledge Article table
CREATE TABLE knowledge_article
(
  id            BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  row_version   BIGINT       NOT NULL DEFAULT 0,
  created_at    TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at    TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by    VARCHAR(100),
  updated_by    VARCHAR(100),

  title         VARCHAR(255) NOT NULL,
  content       TEXT         NOT NULL,
  category      VARCHAR(30)  NOT NULL,
  priority      INT          NOT NULL DEFAULT 0,

  keywords      TEXT[],
  article_type  VARCHAR(50)  NOT NULL DEFAULT 'SOLUTION',
  active        BOOLEAN      NOT NULL DEFAULT true,

  usage_count   BIGINT       NOT NULL DEFAULT 0,
  success_count BIGINT       NOT NULL DEFAULT 0,
  failure_count BIGINT       NOT NULL DEFAULT 0,

  CONSTRAINT chk_kb_category CHECK (category IN
                                    ('BILLING',
                                     'TECHNICAL',
                                     'ACCOUNT',
                                     'SHIPPING',
                                     'SECURITY',
                                     'OTHER',
                                     'PRICING',
                                     'ACCOUNT',
                                     'CANCEL',
                                     'CONTACT',
                                     'DELIVERY',
                                     'FEEDBACK',
                                     'INVOICE',
                                     'ORDER',
                                     'PAYMENT',
                                     'REFUND',
                                     'SHIPPING',
                                     'SUBSCRIPTION'
                                      )),
  CONSTRAINT chk_kb_type CHECK (article_type IN
                                ('SOLUTION',
                                 'FAQ',
                                 'PROCEDURAL_GUIDE',
                                 'TROUBLESHOOTING',
                                 'QUICK_SOLUTION',
                                 'FEATURE_OVERVIEW'
                                  ))
);

CREATE INDEX idx_kb_article_category_active ON knowledge_article (category, active);
CREATE INDEX idx_kb_article_type_active ON knowledge_article (article_type, active);
CREATE INDEX idx_kb_article_priority_active ON knowledge_article (priority DESC, active);
CREATE INDEX idx_kb_article_keywords_gin ON knowledge_article USING GIN (keywords);

COMMENT ON TABLE knowledge_article IS 'Knowledge base articles for automated ticket resolution';
COMMENT ON COLUMN knowledge_article.article_type IS 'SOLUTION, FAQ, PROCEDURAL_GUIDE, TROUBLESHOOTING, QUICK_SOLUTION';

-- Article Template table
CREATE TABLE article_template
(
  id               BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  row_version      BIGINT       NOT NULL DEFAULT 0,
  created_at       TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at       TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by       VARCHAR(100),
  updated_by       VARCHAR(100),

  name             VARCHAR(100) NOT NULL,
  template_content TEXT         NOT NULL,
  variables        JSONB        NOT NULL DEFAULT '[]'::jsonb,

  active           BOOLEAN      NOT NULL DEFAULT true
);

CREATE INDEX idx_article_template_active ON article_template (active);

COMMENT ON TABLE article_template IS 'Response templates for automated replies';

-- Article Template Applicable Categories join table
CREATE TABLE article_template_applicable_categories
(
  article_template_id BIGINT      NOT NULL,
  category            VARCHAR(30) NOT NULL,

  CONSTRAINT pk_article_template_applicable_categories
    PRIMARY KEY (article_template_id, category),

  CONSTRAINT fk_article_template_applicable_categories_template
    FOREIGN KEY (article_template_id)
      REFERENCES article_template (id)
      ON DELETE CASCADE,

  CONSTRAINT chk_article_template_category
    CHECK (category IN ('BILLING', 'TECHNICAL', 'ACCOUNT', 'SHIPPING', 'SECURITY', 'PRICING', 'OTHER'))
);

CREATE INDEX idx_template_categories_template_id
  ON article_template_applicable_categories (article_template_id);

CREATE INDEX idx_template_categories_category
  ON article_template_applicable_categories (category);

-- Article Template Applicable Priorities join table
CREATE TABLE article_template_applicable_priorities
(
  article_template_id BIGINT      NOT NULL,
  priority            VARCHAR(20) NOT NULL,

  CONSTRAINT pk_article_template_applicable_priorities
    PRIMARY KEY (article_template_id, priority),

  CONSTRAINT fk_article_template_applicable_priorities_template
    FOREIGN KEY (article_template_id)
      REFERENCES article_template (id)
      ON DELETE CASCADE

);

CREATE INDEX idx_template_priorities_template_id
  ON article_template_applicable_priorities (article_template_id);

CREATE INDEX idx_template_priorities_priority
  ON article_template_applicable_priorities (priority);

-- Global Config table
CREATE TABLE IF NOT EXISTS global_config
(
  id          BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  row_version BIGINT      NOT NULL DEFAULT 0,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by  VARCHAR(100),
  updated_by  VARCHAR(100),

  config_key  VARCHAR(50) NOT NULL UNIQUE,
  map_value   JSONB       NOT NULL DEFAULT '{}'::jsonb
);

CREATE INDEX IF NOT EXISTS idx_global_config_key_active
  ON global_config (config_key);

COMMENT ON TABLE global_config IS 'Global system configuration flags and values';
COMMENT ON COLUMN global_config.config_key IS 'Unique configuration key (enum)';
COMMENT ON COLUMN global_config.map_value IS 'Configuration value as JSONB map (key-value pairs)';

-- User Settings table
CREATE TABLE user_settings
(
  id                          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  row_version                 BIGINT      NOT NULL DEFAULT 0,
  created_at                  TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at                  TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by                  VARCHAR(100),
  updated_by                  VARCHAR(100),
  user_id                     BIGINT      NOT NULL UNIQUE,
  default_landing             VARCHAR(20) NOT NULL DEFAULT 'DASHBOARD',
  sidebar_collapsed           BOOLEAN     NOT NULL DEFAULT FALSE,
  theme                       VARCHAR(20) NOT NULL DEFAULT 'SYSTEM',
  compact_mode                BOOLEAN     NOT NULL DEFAULT FALSE,
  email_notifications_enabled BOOLEAN     NOT NULL DEFAULT TRUE,
  notify_ticket_reply         BOOLEAN     NOT NULL DEFAULT TRUE,
  notify_status_change        BOOLEAN     NOT NULL DEFAULT TRUE,
  notify_escalation           BOOLEAN     NOT NULL DEFAULT TRUE,
  CONSTRAINT fk_user_settings_user FOREIGN KEY (user_id)
    REFERENCES app_user (id) ON DELETE CASCADE,
  CONSTRAINT chk_user_settings_default_landing
    CHECK (default_landing IN ('DASHBOARD', 'TICKETS', 'QUEUE')),
  CONSTRAINT chk_user_settings_theme
    CHECK (theme IN ('LIGHT', 'DARK', 'SYSTEM'))
);
CREATE UNIQUE INDEX idx_user_settings_user_id ON user_settings (user_id);
COMMENT ON TABLE user_settings IS 'User preferences for workspace behavior, appearance, and notifications';
COMMENT ON COLUMN user_settings.default_landing IS 'Default landing page after login';
COMMENT ON COLUMN user_settings.theme IS 'UI color theme preference';

-- Vector Store table for LLM embeddings and retrieval
CREATE TABLE IF NOT EXISTS vector_store
(
  id        uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  content   text,
  metadata  jsonb,
  embedding vector(768)
);

-- =====================================================
-- NOTE
-- =====================================================
-- This schema is intentionally designed to work cleanly with:
--   @Enumerated(EnumType.STRING)
-- in Hibernate/JPA without needing Postgres enum casting.
-- DB constraints still enforce valid values.
