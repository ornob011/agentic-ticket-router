...
VALUES
  (0,
   ...
   (SELECT now_ts FROM ts),
   'AUTO_ROUTE_THRESHOLD'::config_key_type,
   0.70,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'CRITICAL_MIN_CONF'::config_key_type,
   0.85,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'ROUTER_REPAIR_MAX_RETRIES'::config_key_type,
   2,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'SLA_ASSIGNED_HOURS_HIGH'::config_key_type,
   24,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'WAITING_CUSTOMER_REMINDER_HOURS'::config_key_type,
   48,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'INACTIVITY_AUTO_CLOSE_DAYS'::config_key_type,
   14,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'SLA_CUSTOMER_RESPONSE_HOURS'::config_key_type,
   48,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'SLA_AGENT_RESPONSE_HOURS'::config_key_type,
   24,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'AUTO_CLOSE_WARNING_DAYS'::config_key_type,
   3,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'AUTO_CLOSE_FINAL_DAYS'::config_key_type,
   7,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'MAX_ATTACHMENT_BYTES'::config_key_type,
   52428800,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'AUTO_CLOSE_ENABLED'::config_key_type,
   1,
  ...
  (0,
   (SELECT now_ts FROM ts),
   (SELECT now_ts FROM ts),
   'ROUTER_MODEL_PARAMS'::config_key_type,
   0,
